# Customer User Guide: NvAdPlayer (Nextview TV Client App)

Welcome to the official User Guide for the **NvAdPlayer** application. This guide is written in simple, non-technical language to help you install, activate, and configure the application on your Android TV screens to display video and image advertisements.
...
**NvAdPlayer** is a premium digital signage player application designed to run on Android TVs and media player boxes. It represents the Nextview brand variant of the advertising display engine.
...
### Nextview Scrolling Ticker Marquee
...
The application is distributed as a compiled Android Package (`NvAdPlayer.apk`).
...
4. Select the `NvAdPlayer.apk` file. The app will install automatically on the TV screen.

### Method 2: Manual Installation via USB Pen Drive
1. Copy the `NvAdPlayer.apk` file onto a USB drive.
...
4. Locate the USB storage, click on `NvAdPlayer.apk`, and select **Install**.
...
1. Launch the **NvAdPlayer** app on your TV.
2. The screen will display a unique **Device ID** (e.g. `NVA1B2C3D4`). Copy this ID.
...
1. **File Access:** When the "File Access Required" screen appears, click **Grant Permission**. In the TV settings page that opens, enable the toggle next to **NvAdPlayer** to allow managing all files.
2. **Display Over Other Apps (Overlay):** Go to your TV settings under Apps -> Special App Access -> Display over other apps, and make sure **NvAdPlayer** is set to **Allowed**.
...
* **Speaker Notes:** Welcome. We are presenting NvAdPlayer, the player application that runs directly on your Android TV screens to play advertisement loops.
<!-- slide -->
...
<!-- slide -->
### Slide 6: Nextview Scrolling Ticker
...
<!-- slide -->
### Slide 9: TV Remote Settings drawer
